...
- **Reproducible builds**: `.goreleaser.yaml` uses `metadata.mod_timestamp` and `builds.mod_timestamp` set to `{{ .CommitTimestamp }}`, and `main.date` is set from `{{ .CommitDate }}`.
...
This project is indexed by GitNexus as **actup** (439 symbols, 1005 relationships, 25 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
